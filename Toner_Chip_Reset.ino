...
*/
#define RICOH200      // replace by RICHO150 if required
#include <extEEPROM.h>

#include "reset_bin.h" // the contents to flash on the eeprom to reset the chip.
...
void initialize(void) {
  ...
  byte i2cStat = toner_chip.begin(toner_chip.twiClock100kHz);
...
void reset(void) {
#ifdef RICOH200 
  toner_chip.write(0x0, chip_reset200, chip_dump_len); //buffered write to the chip
#endif
#ifdef RICOH150 
  toner_chip.write(0x0, chip_reset150, chip_dump_len); //buffered write to the chip
#endif
  Serial.println();
  dump();
}
...
void setup(void) {
  ...
  Serial.println("Resetting the chip in 2 seconds");
  delay(2000);
